...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    @Query("SELECT DISTINCT m FROM Message m LEFT JOIN FETCH m.statuses " +
    ...
    @Query("SELECT CASE WHEN COUNT(m) > 0 THEN true ELSE false END FROM Message m " +
            "WHERE (m.senderUuid = :userUuid AND m.receiverEmail = :receiverEmail) " +
            "OR (m.senderEmail = :receiverEmail AND m.receiverUuid = :userUuid)")
    boolean conversationExists(@Param("userUuid") String userUuid, @Param("receiverEmail") String receiverEmail);

    @Query("SELECT m.conversationId FROM Message m " +
            "WHERE (m.senderUuid = :userUuid AND m.receiverEmail = :receiverEmail) " +
            "OR (m.senderEmail = :receiverEmail AND m.receiverUuid = :userUuid) " +
            "ORDER BY m.createdAt DESC")
