...
public class AuthServerClient {
    ...
    public CompletableFuture<Optional<UserRequest>> getUserByEmail(String email) {
        log.info("Calling Auth Server for email: {}", email);

        return CompletableFuture.supplyAsync(() -> {
            UserRequest userInfo = authServerWebClient
                    .get()
            ...
            if (userInfo == null) {
                ...
                return Optional.<UserRequest>empty();
            }
    ...
    @Retry(name = CIRCUIT_BREAKER_NAME)
    @TimeLimiter(name = CIRCUIT_BREAKER_NAME)
    public CompletableFuture<Optional<UserRequest>> getUserByUuid(String userUuid) {
        log.info("Calling Auth Server for UUID: {}", userUuid);

        return CompletableFuture.supplyAsync(() -> {
            UserRequest userInfo = authServerWebClient
                    .get()
                    .uri("/users/{uuid}", userUuid)
                    .retrieve()
                    .onStatus(status -> status == HttpStatus.NOT_FOUND,
                            clientResponse -> Mono.empty())
                    .bodyToMono(UserRequest.class)
    ...
    public CompletableFuture<Optional<UserRequest>> getUserByEmailFallback(String email, Throwable throwable) {
        log.error("Fallback triggered for Auth Server (email: {}): {}", email, throwable.getMessage());
        return CompletableFuture.completedFuture(Optional.empty());
    }

    public CompletableFuture<Optional<UserRequest>> getUserByUuidFallback(String userUuid, Throwable throwable) {
